...
namespace ShelterVault.Services
{
    public class ShelterVaultCosmosDBService : IShelterVaultCosmosDBService
    {
        ...
        public ShelterVaultCosmosDBService(
            ...
            ILogger<ShelterVaultCosmosDBService> logger
        )
        {
        ...
        public async Task UpsertItemAsync<T>(T shelterVault)
        ...
        public async Task DeleteItemAsync<T>(T shelterVault)
            where T : ICosmosDBModel
        {
            if (!IsSyncEnabled())
                return;
            using CosmosClient cosmosClient = GetClient(out CosmosDBSettings cosmosDBSettings);
            Container cosmosContainer = GetContainer(cosmosClient, cosmosDBSettings);
            await cosmosContainer.DeleteItemAsync<object>(
                id: shelterVault.id,
                partitionKey: new PartitionKey(shelterVault.type)
            );
        }
        ...
        public async Task<CosmosDBTinyModel> GetItemByIdAsync(string id)
        {
            ...
            IList<CosmosDBTinyModel> results = await GetCosmosDBItems<CosmosDBTinyModel>(
                new QueryDefinition("SELECT * FROM vault WHERE vault.id = @id").WithParameter(
                    "@id",
                    id
                )
            );
        ...
        public async Task SyncAllAsync(string uuidVault)
        {
            try
            {
                if (!IsSyncEnabled(out ShelterVaultSyncStatusModel shelterVaultSyncStatusModel))
                    return;

                WeakReferenceMessenger.Default.Send(
                    new RefreshCurrentSyncStatusMessage(CloudSyncStatus.SynchInProcess)
                );

                QueryDefinition queryDefinition = GetSyncQueryDefinition(
                    uuidVault,
                    shelterVaultSyncStatusModel
                );
                IList<CosmosDBTinyModel> cosmosDBTinyModels =
                    await GetCosmosDBItems<CosmosDBTinyModel>(queryDefinition);
                IList<VaultModel> shelterVaults = _vaultManager.GetCurrentVaultWithCredentials();
                IList<CosmosDBSyncModel> cosmosDBSyncModels = CosmosDBTinyModel.ToCosmosDBSyncModel(
                    cosmosDBTinyModels
                );
                IList<CosmosDBSyncModel> localDBSyncVaults = VaultModel.ToCosmosDBSyncModel(
                    shelterVaults
                );
                List<CosmosDBSyncModel> syncModels =
                    await cosmosDBSyncModels.SynchronizeVersionsAsync(localDBSyncVaults);
        ...
        {
            ShelterVaultSyncStatusModel shelterVaultSyncStatusModel =
                _shelterVaultSyncStatus.GetSyncStatus(CloudProviderType.Azure);
            CosmosDBSyncStatus currentSyncStatus = new(shelterVaultSyncStatusModel);
            return currentSyncStatus;
        }

        public QueryDefinition GetSyncQueryDefinition(
            string uuidVault,
            ShelterVaultSyncStatusModel shelterVaultSyncStatusModel
        )
        {
            string uuidVaultClause =
                $" WHERE ((vault.type = 'shelter_vault' and vault.id='{uuidVault}') or (vault.type = 'shelter_vault_credentials' and vault.shelterVaultUuid = '{uuidVault}'))";
            string timestampClause =
                shelterVaultSyncStatusModel.Timestamp != 0
                    ? $" and vault._ts > {shelterVaultSyncStatusModel.Timestamp}"
                    : string.Empty;
            string cosmosDBquery = string.Concat(
                "SELECT vault.id, vault.type, vault.version FROM vault",
                uuidVaultClause,
                timestampClause
            );
        ...
        private async Task MakeUpdates(
            IEnumerable<CosmosDBSyncModel> syncModels,
            IEnumerable<CosmosDBSyncModel> cosmosDBSyncModels
        )
        {
            UpsertLocalVaults(
                await GetCosmosDBItemsByPartitionKey(
                    syncModels,
                    ShelterVaultConstants.PARTITION_SHELTER_VAULT
                ),
                cosmosDBSyncModels
            );
            UpsertLocalCredentials(
                await GetCosmosDBItemsByPartitionKey(
                    syncModels,
                    ShelterVaultConstants.PARTITION_SHELTER_VAULT_CREDENTIALS
                ),
                cosmosDBSyncModels
            );
            await UpsertCosmosDbItems(syncModels, ShelterVaultConstants.PARTITION_SHELTER_VAULT);
            await UpsertCosmosDbItems(
                syncModels,
                ShelterVaultConstants.PARTITION_SHELTER_VAULT_CREDENTIALS
            );
            _shelterVaultSyncStatus.UpdateSyncTimestamp(
                CloudProviderType.Azure,
        ...
        private void DeleteManyLocal(
            IEnumerable<CosmosDBSyncModel> synchronizedModels,
            string partitionKey
        )
        {
            foreach (
                string id in synchronizedModels
                    .Where(x =>
                        x.source == SourceType.Local
                        && x.version == -1
                        && x.type.Equals(partitionKey)
                    )
                    .Select(x => x.id)
            )
            {
                if (partitionKey.Equals(ShelterVaultConstants.PARTITION_SHELTER_VAULT))
                    ...
                    partitionKey.Equals(ShelterVaultConstants.PARTITION_SHELTER_VAULT_CREDENTIALS)
                )
                    _shelterVaultCredentials.DeleteCredentials(id);
            }
        }

        private void UpsertLocalVaults(
            IEnumerable<ICosmosDBModel> cosmosDbModels,
            IEnumerable<CosmosDBSyncModel> cosmosDBSyncModels
        )
        {
            foreach (var model in cosmosDbModels)
            {
                ...
                CosmosDBSyncModel cosmosDBModel = cosmosDBSyncModels.FirstOrDefault(x =>
                    x.id.Equals(cosmosDBVault.id)
                );
                if (cosmosDBModel != null && cosmosDBModel.IsNew)
                    _shelterVault.CreateShelterVault(
                        cosmosDBVault.id,
                        cosmosDBVault.name,
                        cosmosDBVault.encryptedTestValue,
                        cosmosDBVault.iv,
                        cosmosDBVault.salt,
                        cosmosDBVault.version
                    );
                else
                    _shelterVault.UpdateShelterVault(
                        cosmosDBVault.id,
                        cosmosDBVault.name,
                        cosmosDBVault.version
                    );
        ...
        private void UpsertLocalCredentials(
            IEnumerable<ICosmosDBModel> cosmosDbModels,
            IEnumerable<CosmosDBSyncModel> cosmosDBSyncModels
        )
        {
            foreach (var model in cosmosDbModels)
            {
                ...
                CosmosDBSyncModel cosmosDBModel = cosmosDBSyncModels.FirstOrDefault(x =>
                    x.id.Equals(cosmosDBCredentials.id)
                );
                if (cosmosDBModel != null && cosmosDBModel.IsNew)
                    _shelterVaultCredentials.InsertCredentials(new(cosmosDBCredentials));
                else
                    _shelterVaultCredentials.UpdateCredentials(new(cosmosDBCredentials));
            }
        }

        private async Task UpsertCosmosDbItems(
            IEnumerable<CosmosDBSyncModel> synchronizedModels,
            string partitionKey
        )
        {
            foreach (
                var id in synchronizedModels
                    .Where(x => x.source == SourceType.Local && x.type == partitionKey)
                    .Select(x => x.id)
            )
            {
                IShelterVaultLocalModel shelterVaultLocalModel = partitionKey.Equals(
                    ShelterVaultConstants.PARTITION_SHELTER_VAULT
                )
                    ? _shelterVault.GetVaultByUUID(id)
                    : _shelterVaultCredentials.GetCredentialsByUUID(id);
                ICosmosDBModel cosmosDBModel = shelterVaultLocalModel.ToCosmosDBModel();
                await UpsertItemAsync(cosmosDBModel);
            }
        }

        private async Task<IEnumerable<ICosmosDBModel>> GetCosmosDBItemsByPartitionKey(
            IEnumerable<CosmosDBSyncModel> cosmosDBModels,
            string partitionKey
        )
        {
            if (!cosmosDBModels.Any())
                return Enumerable.Empty<ICosmosDBModel>();

            string baseQuery = string.Concat(
                partitionKey.Equals(ShelterVaultConstants.PARTITION_SHELTER_VAULT)
            ...
            QueryDefinition query = new QueryDefinition(baseQuery)
                .WithParameter(
                    "@ids",
                    cosmosDBModels
                        .Where(x => x.source == SourceType.CosmosDB && x.type.Equals(partitionKey))
                        .Select(x => x.id)
                )
        ...
        private async Task<IList<T>> GetCosmosDBItems<T>(QueryDefinition query)
            where T : ICosmosDBModel
        {
            using CosmosClient cosmosClient = GetClient(out CosmosDBSettings cosmosDBSettings);
            Container cosmosContainer = GetContainer(cosmosClient, cosmosDBSettings);
        ...
        {
            cosmosDBSettings = _cloudProviderManager.GetCloudConfiguration<CosmosDBSettings>(
                CloudProviderType.Azure
            );
            return new(
                accountEndpoint: cosmosDBSettings.CosmosEndpoint,
                authKeyOrResourceToken: cosmosDBSettings.CosmosKey
            );
        }
        ...
        {
            shelterVaultSyncStatusModel = _shelterVaultSyncStatus.GetSyncStatus(
                CloudProviderType.Azure
            );
            return shelterVaultSyncStatusModel.IsSyncEnabled;
        }
